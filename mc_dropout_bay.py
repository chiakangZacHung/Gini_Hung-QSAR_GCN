...
class mc_dropout():
    ...
    def optimizer(self, lr, opt_type):
        optimizer = tf.train.AdamOptimizer(learning_rate=lr, beta1=self.FLAGS.beta1, beta2=self.FLAGS.beta2,
                                           epsilon=1e-09)
        return optimizer.minimize(self.loss)

    def train(self, A, X, P):
        opt, P_mean, P_logvar, loss = self.sess.run([self.opt, self.P_mean, self.P_logvar, self.loss],
                                                    feed_dict={self.A: A, self.X: X, self.P: P})
        return P_mean, P_logvar, loss

    def test(self, A, X, P):
        P_mean, P_logvar, loss = self.sess.run([self.P_mean, self.P_logvar, self.loss],
                                               feed_dict={self.A: A, self.X: X, self.P: P})
        return P_mean, P_logvar, loss

    def get_feature(self, A, X, P):
        orange= self.sess.run(self.weights,
                          feed_dict={self.A: A, self.X: X, self.P: P})
        return orange
